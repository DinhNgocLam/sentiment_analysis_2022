...
# df_new = pd.read_csv(zf.open('df_resample.csv'))
...
with open(pkl_filename, 'rb') as file:  
    sentiment_analysis_model = pickle.load(zf1.open(file))

pkl_tfidf = "tfidf_model.pkl" 
with open(pkl_tfidf, 'rb') as file:  
    tfidf_model = pickle.load(zf1.open(file))
...
if choice == 'Business Objective':
    ...
    st.write("###### Nhận xét #2: Sau khi tunning parameters với ExtraTreesClassifier, ta được ROC curve và Confusion Matrix như trên với accuracy_score vào khoảng 84.2% (cao hơn so với thuật toán ExtraTreeClassifier).")

    st.write("##### 4. Summary: ExtraTreesClassifier model tạm được để Sentiment Analysis Classification.")
